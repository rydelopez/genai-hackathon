...
def load_dummy_data(db: Session = Depends(get_db)):
    # create instructor and parent
    new_instructor = Instructor(name="Instructor", email="bewill@gmail.com", grade="8", type="instructor")
    db.add(new_instructor)
    db.commit()
    new_parent = Instructor(name="Instructor", email="bewill@gmail.com", child_name="bruh", child_age=4, instructor_id = new_instructor.id, type="parent")
    db.add(new_parent)
    db.commit()
